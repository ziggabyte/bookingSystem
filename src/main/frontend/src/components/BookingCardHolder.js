...
export default function BookingCardHolder({ initialBookings }) {
    ...
    const deleteBooking = (bookingId) => {
        ...
        axios.delete(`http://localhost:8080/api/deleteBooking/${bookingId}`, config)
            .then(({data}) => console.log(data))
        .catch((error) => console.log(error))
         
        const currentBookings = bookings.filter((booking) => booking.bookingId !== bookingId)
            ...
            {bookings.length > 0 ?
                <>
                    ...
                    {bookings.map((booking, index) => {
                            return (
                                <>
                                    <BookingCard booking={booking} deleteBooking={deleteBooking} key={index}/>
                                </>
                            )
                        }   
                    )}
